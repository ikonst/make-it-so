    ...
    public class SolutionParser : SolutionParserBase
    {
        ...
        public override void parse(string solutionFilename)
        {
            try
            {
                ...
                EnvDTE.Projects rootProjects = Utils.dteCall<EnvDTE.Projects>(() => (m_dteSolution.Projects));
        ...
        private void parseProjects(EnvDTE.Projects projects)
        {
            ...
            int numProjects = Utils.dteCall<int>(() => (projects.Count));
            for (int i = 1; i <= numProjects; ++i)
            {
                EnvDTE.Project project = Utils.dteCall<EnvDTE.Project>(() => (projects.Item(i)));
        ...
        private void parseProject(EnvDTE.Project project)
        {
            ...
            string projectName = Utils.dteCall<string>(() => (project.Name));

            // We check if this project is a kind we know how to convert...
            string strProjectType = Utils.dteCall<string>(() => (project.Kind));
            ...
            // projects...
            EnvDTE.ProjectItems projectItems = Utils.dteCall<EnvDTE.ProjectItems>(() => (project.ProjectItems));
        ...
        private void parseProjectItems(EnvDTE.ProjectItems projectItems)
        {
            ...
            int numProjectItems = Utils.dteCall<int>(() => (projectItems.Count));
            for (int i = 1; i <= numProjectItems; ++i)
            {
                EnvDTE.ProjectItem projectItem = Utils.dteCall<EnvDTE.ProjectItem>(() => (projectItems.Item(i)));

                // We see if the item itself has sub-items...
                EnvDTE.ProjectItems subItems = Utils.dteCall<EnvDTE.ProjectItems>(() => (projectItem.ProjectItems));
                if (subItems != null)
                {
                ...
                EnvDTE.Project subProject = Utils.dteCall<EnvDTE.Project>(() => (projectItem.SubProject));
                if (subProject != null)
                {
        ...
        private void parseDependencies()
        {
            ...
            SolutionBuild solutionBuild = Utils.dteCall<SolutionBuild>(() => (m_dteSolution.SolutionBuild));
            BuildDependencies buildDependencies = Utils.dteCall<BuildDependencies>(() => (solutionBuild.BuildDependencies));

            // We loop through the 'BuildDependencies'. Each one of these holds dependency 
            // information for one project...
            int numBuildDependencies = Utils.dteCall<int>(() => (buildDependencies.Count));
            for (int i = 1; i <= numBuildDependencies; ++i)
            {
                BuildDependency buildDependency = Utils.dteCall<BuildDependency>(() => (buildDependencies.Item(i)));

                // We get the project's name...
                EnvDTE.Project project = Utils.dteCall<EnvDTE.Project>(() => (buildDependency.Project));
                string projectName = Utils.dteCall<string>(() => (project.Name));

                // We loop through the required-projects, getting the name of each one...
                object[] requiredProjects = Utils.dteCall<object[]>(() => (buildDependency.RequiredProjects as object[]));
                ...
                for (int j = 0; j < numRequiredProjects; ++j)
                {
                    ...
                    string requiredProjectName = Utils.dteCall<string>(() => (requiredProject.Name));
